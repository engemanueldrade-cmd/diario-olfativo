...
export function perfumeToRow(p) {
  ...
  if (p.volume !== undefined) row.volume = p.volume;
  if (p.climate !== undefined) row.climate = p.climate;
  if (p.occasion !== undefined) row.occasion = p.occasion;
  if (p.alerts !== undefined) row.alerts = p.alerts;
...
// --- Contador de uso mensal da API da Fragella (plano free costuma limitar
// buscas por mês) — guardado no próprio Supabase para não depender de nada
// externo. Uma linha por mês, incrementada a cada busca bem-sucedida.
...
export async function getSearchUsage() {
  ...
  const { data, error } = await supabase.from("api_usage").select("search_count").eq("month", month).maybeSingle();
  if (error) return null;
  return { month, count: data ? data.search_count : 0 };
}
...
export async function incrementSearchUsage() {
  ...
  const { data, error } = await supabase.from("api_usage").select("search_count").eq("month", month).maybeSingle();
  if (error) return null;
  const nextCount = (data ? data.search_count : 0) + 1;
  await supabase.from("api_usage").upsert({ month, search_count: nextCount });
  return { month, count: nextCount };
}
